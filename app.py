...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("lang_"):
        ...
        conn = sqlite3.connect('bot_data.db')
        c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO users (user_id, language) VALUES (?,?)", (user_id, lang))
        conn.commit()
        conn.close()
        await query.edit_message_text(TEXTS[lang]['lang_selected'], reply_markup=get_main_keyboard(lang))
...
async def handle_video_to_mp3(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id, lang):
    ...
    await update.message.reply_text(TEXTS[lang]['audio_ready'] + TEXTS[lang]['send_again'], reply_markup=get_main_keyboard(lang))
...
async def handle_photo_and_audio(update, context, user_id, lang):
    if update.message.photo:
        ...
        image_clip.close()
        audio_clip.close()
        await update.message.reply_video(video=open(out_path, "rb"), reply_markup=get_main_keyboard(lang))
...
async def handle_cut_range(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id, lang):
    ...
    # Progress update simulation (since moviepy write_videofile is blocking, real-time % is complex)
    await update.message.reply_text(TEXTS[lang]['processing'].format(0))
...
def main():
    init_db()
    TOKEN = os.environ.get("858160775:AAFAoUppwpZ-JYl_SmFd6jR-65T5mqxZh74")  # xavfsizroq
    app = Application.builder().token(TOKEN).build()
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CallbackQueryHandler(button))
    app.add_handler(MessageHandler(filters.TEXT, handle_text))
    app.add_handler(MessageHandler(filters.VIDEO | filters.PHOTO | filters.AUDIO | filters.VOICE, handle_media))
